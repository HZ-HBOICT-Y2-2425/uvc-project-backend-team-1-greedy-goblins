...
export async function marketInfoList(req, res) {
  const allMarkets = MarketLocation.map((marketLocationEntry) => {
    ...
    return {
      ...
      marketAdress: `${location.street} ${location.house_number}, ${location.zipcode} ${location.city}`,
      marketStreet: location.street,
      marketHouseNumber: location.house_number,
      marketZipcode: location.zipcode,
      marketCity: location.city,
